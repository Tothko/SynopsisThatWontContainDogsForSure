...
{
	if (strcmp (fullTypeName, "NvidiaHack.TheHack") == 0) return 1000000 ; 
	return 0 ;
}
extern "C" DLL_PUBLIC const char* HybridizerGetTypeFromID( const int typeId)
{
	if (typeId == 1000000) return "NvidiaHack.TheHack" ; 
	return "" ;
}
extern "C" DLL_PUBLIC int HybridizerGetShallowSize (const char* fullTypeName) 
{
	#ifdef __TYPE_DECL__NvidiaHack_TheHack___
	if (strcmp (fullTypeName, "NvidiaHack.TheHack") == 0) return 8 ; 
	#endif
	return 0 ;
}
